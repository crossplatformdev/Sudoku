...
int EmpezarPartida(struct sudoku *sk) {
	...
	do {
		...
		if (error == 0)
			return error;
	} while (error != 1); // Si consiguio leer el archivo anterior, intentara abrir la partida. Si no, vuelve al menu principal.
	...
	if (sk->espacios == 0)
		return -1; // Si un fichero estuviera compuesto completamente por numeros, esto volveria al menu inicial.
	return error;
}
...
int Jugar(struct sudoku* sk) {
	...
	do {
		...
		if (error == -1)
			return error; // Si error == -1 Salir
		if (error == -2) {
			GuardaTablero(*sk);
			return error;
		} // Si error == -2, guarda la partida.
		if (error == -3) {
			ModificaNumero(sk, fc);
			return error;
		} // Si error == -3, modifica un numero.
		if (error == -4) {
			LimpiaTablero(sk);
			return error;
...
void DibujarTablero(struct sudoku* sk) //No tiene misterio. Dibuja el tablero e imprime las estadisticas.
{
	...
	for (i = MIN_FILA; i <= MAX_FILA; i++) {
		...
		if ((i - CHAR) % FILAS == 0 || (i - CHAR) % FILAS == 3
				|| (i - CHAR) % FILAS == 6) {
			printf("\t\t\t- |- - -|- - -|- - -|\n");
		}
		printf("\t\t\t%c ", i);
		for (j = MIN_COLUMNA; j <= MAX_COLUMNA; j++) {
			...
			if ((j - INT) % COLUMNAS == 0 || (j - INT) % COLUMNAS == 3
					|| (j - INT) % COLUMNAS == 6) {
				printf("|");
			}
			printf("%c", sk->panel[i - CHAR][j - INT]);

			if ((j - INT) % COLUMNAS != 2 && (j - INT) % COLUMNAS != 5
					&& (j - INT) % COLUMNAS != 8) {
				printf("-");
			}
		}
	...
	printf(
			...
			sk->partida, sk->espacios,
			100.0 - ((float) sk->espacios * 100 / (CASILLAS)));
...
int LeeTablero(struct sudoku* sk) // Abre partidas de sudokus (puzzles.txt o almacenadas.txt) e inicializa el juego.
{
	...
	if (partida == NULL) {
		printf(
				"ERROR AL LEER EL ARCHIVO DE PARTIDA.\nCompruebe que los archivos esten en el directorio del ejecutable.\n\a");
		error = 0;
	} else {
		for (i = 0; i < FILAS; i++) {
			for (j = 0; j < COLUMNAS; j++) {
				fscanf(partida, "\n%c", &sk->panel[i][j]); // En este bucle se inicializa el sudoku
				if (sk->panel[i][j] == PUNTO)
					sk->espacios++; // y se actualiza el numero de espacios.
			}
		}
		error = 1;
		fclose(partida);
...
//Comprueba si las coordenadas son correctas. Si son "Coordenadas-Comando" las identifica y hace lo que corresponda.
	...
	if (strlen(coord) != 2) {
		printf(
				"Debes escribir en formato (LN [LineaColumna, sin espacios y en los margenes permitidos]).\n\a");
		error = 0;
	}
...
int CasillaVacia(struct sudoku sk, struct coor fc) {
	...
	if ((sk.panel[fc.fila - CHAR][fc.columna - INT] == PUNTO)
		...
		printf(
				"La casilla ya ha sido rellenada con un numero. Elige otra.\n\a");
		error = 0;
	}
...
//Inserta un numero si las coordenadas son correctas y no hay un numero ya introducido en ellas.
	...
	do {
		while (error == 0) {
			...
			printf(
					"Introduce una fila A-I e introduce una columna 1-9...(Formato LC).\t");
			scanf("\n%s", coord);
			fflush(stdin);
			fc.fila = coord[0];
			fc.columna = coord[1];
	...
	do {
		...
		if (num == 'C') {
			printf("\nOperacion Cancelada. \n\a");
			return 0;
		} // 'C' Cancela la introduccion del numero, vuelve a pedir coordenadas.
		error = CompruebaNumero(&num);
		if (error == 1)
			error = NumeroValido(*sk, coord, &num);
...
int NumeroValido(struct sudoku sk, char coord[], char * num) {
	...
	if (((int) (fila - CHAR) % 9 == 0) || ((int) (fila - CHAR) % 9 == 1)
			|| ((int) (fila - CHAR) % 9 == 2))
		sub_cuadrante[0] = 'A';
	if (((int) (fila - CHAR) % 9 == 3) || ((int) (fila - CHAR) % 9 == 4)
			|| ((int) (fila - CHAR) % 9 == 5))
		sub_cuadrante[0] = 'D';
	if (((int) (fila - CHAR) % 9 == 6) || ((int) (fila - CHAR) % 9 == 7)
			|| ((int) (fila - CHAR) % 9 == 8))
		sub_cuadrante[0] = 'G';

	if (((int) (columna - INT) % 9 == 0) || ((int) (columna - INT) % 9 == 1)
			|| ((int) (columna - INT) % 9 == 2))
		sub_cuadrante[1] = '1';
	if (((int) (columna - INT) % 9 == 3) || ((int) (columna - INT) % 9 == 4)
			|| ((int) (columna - INT) % 9 == 5))
		sub_cuadrante[1] = '4';
	if (((int) (columna - INT) % 9 == 6) || ((int) (columna - INT) % 9 == 7)
			|| ((int) (columna - INT) % 9 == 8))
		sub_cuadrante[1] = '7';

	for (f = (sub_cuadrante[0] - CHAR); f <= (sub_cuadrante[0] - CHAR) + 2; f++) // Comparamos num con los numeros del subcuadrante.
			{
		for (c = (sub_cuadrante[1] - INT); c <= (sub_cuadrante[1] - INT) + 2;
				c++) {
...
int LeerPartidasGuardadas(struct sudoku* sk) {
	...
	if (salvadas == NULL) {
		printf(
				"ERROR AL LEER EL ARCHIVO \"almacenadas.txt\".\nCompruebe que los archivos esten en el directorio del ejecutable.\n\a");
		error = 0;
	} else {
		...
		do {
			...
			while (!feof(salvadas)) // Muestra los nombres de partidas guardadas en almacenadas.txt. SIEMPRE APARECE EL ULTIMO NOMBRE DOS VECES!!??
			{
				...
				fscanf(salvadas, "\n%s", partidas);
				fflush(stdin);
				if (fscanf(salvadas, "\EOF", partidas))
					break;
				printf("\t%s", partidas);
				printf("\n");
			}
...
int ModificaNumero(struct sudoku* sk, struct coor fc) {
	...
	do {
		while (error == 0) {
			...
			printf(
					"\nOpciones: CN=CambiarNumero LT=LimpiarTablero GP=GuardarPartida SA=SAlir\n\n");
			printf(
					"Introduce una fila A-I e introduce una columna 1-9.\n(Formato LC) del numero que quieras modificar: \t");
	...
	do {
		printf(
				"Ahora introduce un valor 1-9 o punto para borrar...(C=Cancelar)\t");
		scanf("\n%c", &num);
		fflush(stdin);
		if (num == 'C') {
			printf("\nOperacion Cancelada. \n\a");
